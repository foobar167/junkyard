...
class FeatureExtractor:
    ...
    def __init__(self, impath=None):
        self.__extractor = cv2.AKAZE_create()  # initiate AKAZE feature extractor
        self.__detect_and_compute = self.__detect_and_compute_1
        self.__nn_match_ratio = 0.7  # nearest neighbor matching ratio
        self.__matches = 6  # number of good matches to draw quadrilateral
    ...
    def __detect_and_compute_1(self, gray):
        ...
        return self.__extractor.detectAndCompute(gray, None)
    ...
    def __detect_and_compute_2(self, gray):
        ...
        return self.__extractor.compute(gray, keypoints)  # compute the descriptors with BRIEF
    ...
    def __compute(self, image):
        ...
        keypoints, descriptor = self.__detect_and_compute(gray)
    ...
    def tracking(self, image):
        ...
        for i, (m, n) in enumerate(matches):
            if m.distance < self.__nn_match_ratio * n.distance:
                matches_mask[i] = [1, 0]
                good_matches.append(m)

        if len(good_matches) > self.__matches:  # draw a quadrilateral if there are enough matches
