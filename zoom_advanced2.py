...
class Zoom_Advanced(ttk.Frame):
    ...
    def show_image(self, event=None):
        ...
        bbox = list(map(int, bbox1))  # convert to integer, otherwise it will not work properly
        bbox = [min(bbox[0], bbox2[0]), min(bbox[1], bbox2[1]),  # get scroll region box
                max(bbox[2], bbox2[2]), max(bbox[3], bbox2[3])]
        ...
        if  bbox[1] == bbox2[1] and bbox[3] == bbox2[3]:  # whole image in the visible area
            bbox[1]  = bbox1[1]
            bbox[3]  = bbox1[3]
        self.canvas.configure(scrollregion=tuple(map(int, bbox)))  # set integer scroll region
