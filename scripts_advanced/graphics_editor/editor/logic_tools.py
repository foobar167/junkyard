...
def get_images(imframe, config):
    ...
    for i, c in enumerate(imframe.roi_dict.values()):
        im = imframe.crop((c[0], c[1], c[2], c[3]))  # cut sub-rectangle from the image
        # Create filename
        imname = ('{name}_{i:0'+n+'}_' +
                  '{c1:0'+m+'}-{c0:0'+m+'}-' +
                  '{c3:0'+m+'}-{c2:0'+m+'}.png').format(
            name=name, i=i, c0=c[0], c1=c[1], c2=c[2], c3=c[3])
