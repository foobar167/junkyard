...
class Zoom_Advanced(ttk.Frame):
    ...
    def __init__(self, mainframe, path):
        ...
        self.edge = None  # edge of a polygon
        #id = self.canvas.create_polygon(10, 10, 200, 200, 100, 250, 150, 180, 30, 120, fill='', outline='gray', width=2)
        #id = self.canvas.create_line(15, 15, 220, 220, 120, 270, 170, 200, width=2)
        #
        self.show_image()
    ...
    def set_edge(self, event):
        ...
        if self.edge == None:  # start drawing polygon
            self.edge = self.canvas.create_line(x, y, x, y, fill='blue', width=2, tag=self.edge_tag_start)
        else:
            x1, y1, x2, y2 = self.canvas.coords(self.edge_tag_start)  # get coordinates of the 1st edge
            x3, y3, x4, y4 = self.canvas.coords(self.edge)  # get old coordinates of the edge
            if x4 == x1 and y4 == y1:  # finish drawing polygon
                ...
                self.edge = self.canvas.create_line(x, y, x, y, fill='blue', width=2, tag=self.edge_tag)
    ...
    def motion(self, event):
        ...
        if self.edge:  # relocate edge of a polygon
            x = self.canvas.canvasx(event.x)  # get coordinates of the event on the canvas
            y = self.canvas.canvasy(event.y)
            x1, y1, x2, y2 = self.canvas.coords(self.edge_tag_start)# get coordinates of the 1st edge
            x3, y3, x4, y4 = self.canvas.coords(self.edge)  # get old coordinates of the edge
            ...
            if self.sticking_radius * self.sticking_radius > dx * dx + dy * dy:
                self.canvas.coords(self.edge, x3, y3, x1, y1)
            else:
                self.canvas.coords(self.edge, x3, y3, x, y)
